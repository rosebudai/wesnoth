#!/usr/bin/env bash
# Docker-based test: verify Wesnoth web add-on manager connects and loads add-ons.
# Runs serve_coi.py, ws_proxy_node.js, and Playwright all inside a single Docker container
# so they share the same network namespace (localhost works for all).
#
# Usage: ./test_addons_docker.sh [screenshot_dir]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../../.." && pwd)"
BUILD_DIR="${REPO_ROOT}/utils/dockerbuilds/emscriptenbuild"
SCREENSHOT_DIR="${1:-/tmp/wesnoth-addon-docker-test}"
...
echo "=== Wesnoth Web Add-on Docker Test ==="
...
docker run --rm \
  ...
  bash -lc '
    ...
    echo "=== Running Playwright add-on test ==="
    node ./tests/test_addons_playwright.js http://127.0.0.1:8040/ /screenshots
  '
